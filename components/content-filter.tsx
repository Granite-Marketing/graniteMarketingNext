...
export function ContentFilter({ categories, selectedCategories, onCategoryChange, label = "topic" }: ContentFilterProps) {
  const [isOpen, setIsOpen] = useState(false)

  const capitalizedLabel = label.charAt(0).toUpperCase() + label.slice(1)
  const capitalizedLabelPlural = capitalizedLabel + "s"
          ...
          <DropdownMenuTrigger asChild>
            ...
            >
              <span>
                {isAllSelected
                  ? `All ${capitalizedLabelPlural}`
                  : `${selectedCategories.length} ${selectedCategories.length === 1 ? label : labelPlural} selected`}
              </span>
