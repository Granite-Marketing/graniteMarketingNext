import { render, screen } from "@testing-library/react";
import { beforeEach, describe, expect, it, vi } from "vitest";
import type { Section } from "@/lib/sanity/lib/page-sections";

// U21 of the Sanity page builder plan: /blog renders its ContentHero chrome
// (tag/heading/subtitle) and sectionsAbove/sectionsBelow from blogListing
// once it is PUBLISHED, falling back to today's hardcoded chrome otherwise
// — the same zero-risk cutover shape app/page.tsx established for the
// homepage (U16). BlogGrid is the fixed region: it has no field on
// blogListing and must render in BOTH branches unconditionally.
vi.mock("@/components/nav", () => ({
	Nav: () => <div data-testid="nav-stub" />,
}));
vi.mock("@/components/footer", () => ({
	Footer: () => <div data-testid="footer-stub" />,
}));
vi.mock("@/components/cta", () => ({
	CTA: () => <div data-testid="hardcoded-cta-stub" />,
}));
vi.mock("@/components/blog-grid", () => ({
	BlogGrid: () => <div data-testid="blog-grid-stub" />,
}));

const contentHeroSpy = vi.fn();
vi.mock("@/components/content-hero", () => ({
	ContentHero: (props: unknown) => {
		contentHeroSpy(props);
		return <div data-testid="content-hero-stub" />;
	},
}));

const pageBuilderSpy = vi.fn();
vi.mock("@/components/page-builder", () => ({
	PageBuilder: (props: unknown) => {
		pageBuilderSpy(props);
		return <div data-testid="page-builder-stub" />;
	},
}));

const getBlogPosts = vi.fn();
const getBlogListingPublished = vi.fn();
const getBlogListing = vi.fn();
const getPageCtaDefaults = vi.fn();

vi.mock("@/lib/sanity/queries", () => ({
	getBlogPosts: (...args: unknown[]) => getBlogPosts(...args),
	getBlogListingPublished: (...args: unknown[]) =>
		getBlogListingPublished(...args),
	getBlogListing: (...args: unknown[]) => getBlogListing(...args),
	getPageCtaDefaults: (...args: unknown[]) => getPageCtaDefaults(...args),
}));

import BlogPage, { generateMetadata } from "../page";

const HERO_SECTION: Extract<Section, { _type: "heroBlock" }> = {
	_key: "hero-1",
	_type: "heroBlock",
	anchorId: null,
	eyebrow: null,
	heading: "Sanity hero heading",
	body: null,
	primaryCtaLabel: null,
	secondaryCta: null,
	showTrustedBy: false,
};

const FALLBACK_TAG = "Blog & Insights";
const FALLBACK_HEADING = "Automation insights that matter";
const FALLBACK_SUBTITLE =
	"Stay ahead with the latest strategies, case studies, and best practices in AI-powered workflow automation.";
const FALLBACK_TITLE = "Blog - Granite Marketing | AI Automation Insights";
const FALLBACK_DESCRIPTION =
	"Discover the latest insights, tutorials, and best practices in AI automation, workflow optimization, and business process improvement.";

function listingDoc(overrides: Record<string, unknown> = {}) {
	return {
		_id: "blogListing",
		_type: "blogListing",
		seo: null,
		tag: "Custom Tag",
		heading: "Custom Heading",
		subtitle: "Custom subtitle copy.",
		sectionsAbove: [],
		sectionsBelow: [],
		...overrides,
	};
}

beforeEach(() => {
	vi.clearAllMocks();
	getBlogPosts.mockResolvedValue([]);
	getPageCtaDefaults.mockResolvedValue(null);
	// blogListing is a draft-only document today — the default here mirrors
	// that, exactly like app/page.tsx's getHomePageSlug default.
	getBlogListingPublished.mockResolvedValue(false);
});

describe("app/blog — rendering", () => {
	it("renders the hardcoded chrome when blogListing has no published version", async () => {
		getBlogListingPublished.mockResolvedValue(false);

		const jsx = await BlogPage();
		render(jsx);

		expect(contentHeroSpy).toHaveBeenCalledWith(
			expect.objectContaining({
				tag: FALLBACK_TAG,
				heading: FALLBACK_HEADING,
				subtitle: FALLBACK_SUBTITLE,
			})
		);
		expect(screen.getByTestId("hardcoded-cta-stub")).toBeInTheDocument();
		expect(screen.queryByTestId("page-builder-stub")).not.toBeInTheDocument();
		expect(getBlogListing).not.toHaveBeenCalled();
	});

	it("the fixed BlogGrid renders even when unpublished", async () => {
		getBlogListingPublished.mockResolvedValue(false);

		const jsx = await BlogPage();
		render(jsx);

		expect(screen.getByTestId("blog-grid-stub")).toBeInTheDocument();
	});

	it("renders the document's hero copy and sections when blogListing is published", async () => {
		getBlogListingPublished.mockResolvedValue(true);
		getBlogListing.mockResolvedValue(
			listingDoc({ sectionsAbove: [HERO_SECTION], sectionsBelow: [] })
		);

		const jsx = await BlogPage();
		render(jsx);

		expect(contentHeroSpy).toHaveBeenCalledWith(
			expect.objectContaining({
				tag: "Custom Tag",
				heading: "Custom Heading",
				subtitle: "Custom subtitle copy.",
			})
		);
		expect(pageBuilderSpy).toHaveBeenCalledWith(
			expect.objectContaining({
				documentId: "blogListing",
				documentType: "blogListing",
				sections: [HERO_SECTION],
			})
		);
		expect(screen.queryByTestId("hardcoded-cta-stub")).not.toBeInTheDocument();
	});

	it("the fixed BlogGrid renders even when published — the region an editor cannot remove", async () => {
		getBlogListingPublished.mockResolvedValue(true);
		getBlogListing.mockResolvedValue(listingDoc());

		const jsx = await BlogPage();
		render(jsx);

		expect(screen.getByTestId("blog-grid-stub")).toBeInTheDocument();
	});

	it("falls back to today's literals per field when the published document's hero fields are empty, not to empty strings", async () => {
		getBlogListingPublished.mockResolvedValue(true);
		getBlogListing.mockResolvedValue(
			listingDoc({ tag: null, heading: "", subtitle: null })
		);

		const jsx = await BlogPage();
		render(jsx);

		expect(contentHeroSpy).toHaveBeenCalledWith(
			expect.objectContaining({
				tag: FALLBACK_TAG,
				heading: FALLBACK_HEADING,
				subtitle: FALLBACK_SUBTITLE,
			})
		);
	});

	it("omits sectionsAbove/sectionsBelow PageBuilder instances when both are empty", async () => {
		getBlogListingPublished.mockResolvedValue(true);
		getBlogListing.mockResolvedValue(
			listingDoc({ sectionsAbove: [], sectionsBelow: [] })
		);

		const jsx = await BlogPage();
		render(jsx);

		expect(pageBuilderSpy).not.toHaveBeenCalled();
	});

	it("falls back to the hardcoded chrome if getBlogListing returns null despite a published check", async () => {
		getBlogListingPublished.mockResolvedValue(true);
		getBlogListing.mockResolvedValue(null);

		const jsx = await BlogPage();
		render(jsx);

		expect(contentHeroSpy).toHaveBeenCalledWith(
			expect.objectContaining({ tag: FALLBACK_TAG })
		);
		expect(screen.getByTestId("hardcoded-cta-stub")).toBeInTheDocument();
	});
});

describe("app/blog — generateMetadata", () => {
	it("falls back to the current hardcoded metadata when blogListing has no published version", async () => {
		getBlogListingPublished.mockResolvedValue(false);

		const metadata = await generateMetadata();

		expect(metadata).toEqual({
			title: FALLBACK_TITLE,
			description: FALLBACK_DESCRIPTION,
		});
		expect(getBlogListing).not.toHaveBeenCalled();
	});

	it("sources title/description from seo when blogListing is published", async () => {
		getBlogListingPublished.mockResolvedValue(true);
		getBlogListing.mockResolvedValue(
			listingDoc({
				seo: { metaTitle: "Custom Blog Title", metaDescription: "Custom blog description" },
			})
		);

		const metadata = await generateMetadata();

		expect(metadata.title).toBe("Custom Blog Title");
		expect(metadata.description).toBe("Custom blog description");
	});

	it("falls back per-field to the hardcoded metadata when seo is unset", async () => {
		getBlogListingPublished.mockResolvedValue(true);
		getBlogListing.mockResolvedValue(listingDoc({ seo: null }));

		const metadata = await generateMetadata();

		expect(metadata.title).toBe(FALLBACK_TITLE);
		expect(metadata.description).toBe(FALLBACK_DESCRIPTION);
	});

	it("keeps the fallback description when only metaTitle is filled in", async () => {
		getBlogListingPublished.mockResolvedValue(true);
		getBlogListing.mockResolvedValue(
			listingDoc({ seo: { metaTitle: "Only A Title", metaDescription: null } })
		);

		const metadata = await generateMetadata();

		expect(metadata.title).toBe("Only A Title");
		expect(metadata.description).toBe(FALLBACK_DESCRIPTION);
	});

	it("strips stega characters from the resolved metadata", async () => {
		const STEGA_MARKER = "​​​​";
		getBlogListingPublished.mockResolvedValue(true);
		getBlogListing.mockResolvedValue(
			listingDoc({
				seo: {
					metaTitle: `Stega${STEGA_MARKER}Title`,
					metaDescription: `Stega${STEGA_MARKER}Description`,
				},
			})
		);

		const metadata = await generateMetadata();

		expect(metadata.title).toBe("StegaTitle");
		expect(metadata.description).toBe("StegaDescription");
	});
});
